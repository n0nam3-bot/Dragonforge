...
/**
 * Render every animation frame and pack into a sprite sheet.
 * @param {object} bodyData   — from detectBodyParts()
 * @param {string} pose
 * @param {string} direction  — 'left' | 'right'
 * @param {number} frameCount
 * @param {number} frameSize  — px per frame (square)
 * @param {string} layout     — 'horizontal' | 'grid'
 * @returns {HTMLCanvasElement}
 */
export function bakeSheet(bodyData, pose, direction, frameCount, frameSize, layout) {
  ...
  for (let f = 0; f < frameCount; f++) {
    ...
    renderFrame(tCtx, bodyData, pose, t, direction);
...
export function exportPNG(sheetCanvas, pose, direction) {
  sheetCanvas.toBlob(blob => {
    ...
    const a   = Object.assign(document.createElement('a'), {
      href: url, download: `spritesmith_${pose}_${direction}.png`
    });
    a.click();
...
// ── Export JSON (Aseprite array format) ──────────────────────────────────────
...
export function exportJSON(sheetCanvas, frameCount, frameSize, layout, pose, direction) {
  ...
  const frames = Array.from({ length: frameCount }, (_, f) => ({
    ...
    frame:    { x: (f % cols) * frameSize, y: Math.floor(f / cols) * frameSize,
  ...
  const meta = {
    app: 'SpriteSmith Studio', version: '2.0',
