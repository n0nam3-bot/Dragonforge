...
const MAX_SIDE    = 900;   // resize before processing if larger
const BG_THOLD    = 42;    // colour-distance threshold for bg detection
const SEMI_THOLD  = 70;    // wider pass for semi-transparent fringe cleanup
...
export async function removeBackground(imgEl, onProgress = () => {}) {
  ...
  let existingAlpha = 0;
  for (let i = 3; i < d.length; i += 4) if (d[i] < 200) existingAlpha++;
  if (existingAlpha / (W * H) > 0.08) { onProgress(1); return orig; }

  // ---- 3. Sample edges to find background colour ----
  const bgColour = sampleEdgeBg(d, W, H);
  onProgress(0.15);
  await tick();

  // ---- 4. Flood-fill from all edge pixels ----
  const mask = new Uint8Array(W * H); // 1 = background
  floodFillEdges(d, mask, W, H, bgColour, BG_THOLD);
  onProgress(0.60);
  await tick();

  // ---- 5. Widen pass: semi-transparent fringe close to bg colour ----
  for (let i = 0; i < W * H; i++) {
    if (mask[i]) continue;
    const px = px4(d, i);
    if (colourDist(px, bgColour) < SEMI_THOLD) mask[i] = 2; // fringe
  }
  onProgress(0.70);
  await tick();

  // ---- 6. Write alpha: bg → 0, fringe → partial ----
  for (let i = 0; i < W * H; i++) {
    if (mask[i] === 1) { d[i*4+3] = 0; }
    else if (mask[i] === 2) { d[i*4+3] = Math.round(d[i*4+3] * 0.25); }
  }
  onProgress(0.80);
  await tick();

  // ---- 7. Smooth alpha edges (1-px Gaussian on alpha channel) ----
  smoothAlpha(d, W, H);
  onProgress(0.92);
  await tick();
...
function floodFillEdges(d, mask, W, H, bgCol, thold) {
  // Use a typed Uint32Array as a fast ring-buffer queue
